...
"""Shared fixtures for behavior_calibration tests."""
...
def completed_index(tmp_path: Path):
    payload = make_manifest_payload(artifacts=[_base_entry(), _singleton_entry()])
...
def empty_index(tmp_path: Path):
    payload = make_manifest_payload(artifacts=[])
    manifest_path = write_manifest(tmp_path, payload)
    manifest = load_artifact_manifest(manifest_path)
    return build_artifact_index(manifest, manifest_path)
...
@pytest.fixture()
def index_with_excluded_paths(tmp_path: Path):
    payload = make_manifest_payload(
        artifacts=[_base_entry()],
        excluded_paths=[
            {"path": "coverage.ini", "reason": "noise", "pattern": "coverage.ini"},
            {"path": ".coverage.abc", "reason": "noise", "pattern": ".coverage*"},
        ],
    )
    manifest_path = write_manifest(tmp_path, payload)
    manifest = load_artifact_manifest(manifest_path)
...
@pytest.fixture()
def index_from_example_completed():
