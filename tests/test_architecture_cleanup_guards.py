...
def test_executor_stack_removed_from_source_tree() -> None:
    assert not (REPO_ROOT / "src" / "control_plane" / "adapters" / "executor").exists()
...
def test_removed_modules_cannot_be_imported() -> None:
    with pytest.raises(ModuleNotFoundError):
        importlib.import_module("control_plane.adapters.executor.factory")
    with pytest.raises(ModuleNotFoundError):
        importlib.import_module("control_plane.legacy_execution")


def test_routing_client_no_longer_supports_in_process_bypass() -> None:
    source = (REPO_ROOT / "src" / "control_plane" / "routing" / "client.py").read_text(encoding="utf-8")
...
def test_no_execution_proxy_env_injection_in_canonical_backends() -> None:
    paths = [
        REPO_ROOT / "src" / "control_plane" / "backends" / "kodo" / "invoke.py",
        REPO_ROOT / "src" / "control_plane" / "backends" / "archon" / "invoke.py",
        REPO_ROOT / "src" / "control_plane" / "backends" / "direct_local" / "adapter.py",
    ]
    for path in paths:
