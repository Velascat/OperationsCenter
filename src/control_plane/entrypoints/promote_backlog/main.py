...
Usage:
    ...
    python -m control_plane.entrypoints.promote_backlog.main --config FILE

    # Execute: actually transition tasks
    python -m control_plane.entrypoints.promote_backlog.main --config FILE --execute

    # Limit to one family
    python -m control_plane.entrypoints.promote_backlog.main --config FILE --family lint_fix --execute
"""
...
from control_plane.autonomy_tiers.config import get_family_tier, load_tiers_config
from control_plane.config import load_settings
from control_plane.proposer.backlog_promoter import BacklogPromoterService
...
def main() -> None:
    ...
    parser.add_argument("--config", required=True, help="Path to control_plane config YAML")
    parser.add_argument(
        "--execute",
        action="store_true",
