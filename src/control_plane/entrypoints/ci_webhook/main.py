...
This eliminates the polling delay between CI completing and ControlPlane acting.

Security: all incoming requests are validated against the ``X-Hub-Signature-256``
HMAC header using the secret from ``CONTROL_PLANE_WEBHOOK_SECRET``.  Requests
without a valid signature are rejected with HTTP 401.

Environment variables:
  CONTROL_PLANE_WEBHOOK_SECRET   — HMAC secret set in GitHub webhook settings
  CONTROL_PLANE_WEBHOOK_PORT     — Port to listen on (default: 8765)
  CONTROL_PLANE_WEBHOOK_HOST     — Host to bind to (default: 127.0.0.1)
  CONTROL_PLANE_WEBHOOK_TRIGGER  — Optional command to run on a CI event
                                   (default: write a trigger file)

Usage::

    python -m control_plane.entrypoints.ci_webhook.main
...
_SECRET_ENV = "CONTROL_PLANE_WEBHOOK_SECRET"
_PORT_ENV = "CONTROL_PLANE_WEBHOOK_PORT"
_HOST_ENV = "CONTROL_PLANE_WEBHOOK_HOST"
_TRIGGER_ENV = "CONTROL_PLANE_WEBHOOK_TRIGGER"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Receive GitHub check_run webhooks and trigger ControlPlane cycles."
    )
