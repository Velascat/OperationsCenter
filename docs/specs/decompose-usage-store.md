...
- **Backward-compatible public API**: `from operations_center.execution.usage_store import UsageStore` and `from operations_center.execution import UsageStore` must continue to work. `UsageStore` retains all 44 method signatures via thin delegation (e.g., `self._budget.budget_decision(now=now)`). No caller changes required.
- **`_check_disk_space` re-export**: `usage_store.py` must re-export `_check_disk_space` from `_utils` so that the 4 existing `from operations_center.execution.usage_store import _check_disk_space` call sites keep working without modification.
...
- `ruff check src/operations_center/execution/` reports no new lint violations.
