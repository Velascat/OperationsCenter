...
|----|---------|---------------|--------|--------|-----|
| F1 | Stale `Running` task auto-recovery | A worker dies mid-task → Plane shows Running forever; ghost-audit detects but nothing reclaims | Operator must manually reset; queue stalls if it's the only worker per role | **Fixed** | `entrypoints/maintenance/recover_stale.py` resets Running tasks older than 4h (configurable via `--max-age-seconds`; `--per-kind` for per-task-kind TTLs) to `Ready for AI` with an audit comment |
