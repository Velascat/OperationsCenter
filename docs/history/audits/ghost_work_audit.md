...
| G8 | Stale Running tasks | Worker dies mid-task (OOM, restart, kodo hang); Plane shows Running forever | No timeout-based reclaim | **Fixed** | `entrypoints/maintenance/recover_stale.py` resets Running tasks older than `--max-age-seconds` (default 4h) to `Ready for AI` with an audit comment; supports `--per-kind` for per-task-kind TTLs |
