...
A live-refresh terminal status dashboard for OperationsCenter operators. Reads only local files — no Plane API calls at display time. Supports optional repo filtering.
...
**Location:** `src/operations_center/entrypoints/worker/main.py` — new function `write_board_snapshot()`, called from the main watcher cycle wherever `list_issues()` results are available.
...
```json
{
  ...
  "written_by": "goal",
  "counts": {
    "OperationsCenter": {"Running": 1, "Ready for AI": 2, "Blocked": 0, "Review": 0},
...
| Circuit breaker | `tools/report/operations_center/execution/usage.json` |
...
**Location:** `scripts/operations-center.sh` — new `status` command.

```bash
operations-center.sh status [--repo OperationsCenter,ExternalRepo]
```
...
```
────────── OperationsCenter Status  [2026-04-20 02:14:33 UTC]  (Ctrl+C to exit) ──
...
ACTIVE KODO
  ...
  repo: OperationsCenter   branch: plane/034e283e-…   elapsed: 3m 24s
...
**Syntax:** `--repo OperationsCenter` or `--repo OperationsCenter,ExternalRepo`
...
Read `tools/report/operations_center/execution/usage.json`, filter to `kind == "execution_outcome"` events within the last `_CB_STALENESS_HOURS` (4h default), take last 5. Compute failure rate. Show `open` or `closed` with fraction and threshold.
...
| File | Change |
|---|---|
| `src/operations_center/entrypoints/worker/main.py` | Add `write_board_snapshot()`, call it from watcher cycle |
